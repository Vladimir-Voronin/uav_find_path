...
class Node:
    def __init__(self, point_expand, g, prev_node, coordinate_int_x, coordinate_int_y):
...
class DijkstraMethodGrid(AlgoritmsBasedOnHallAndGrid, SearchAlgorithm, ABC):
        ...
        while full_time < self.time_to_succeed:
            ...
            if self.__check_distance_to_target_point(current_node) < self.point_search_distance_diagonal:
                ...
                if geometry.distance(line):
                    self.last_node = Node(self.target_point_expand, 0, current_node, 0, 0)
                    self.is_succes = True
                    break
            self.open_list.remove(current_node)
            self.closed_list.append(current_node)
...
if __name__ == '__main__':
    ...
    for i in range(n):
        ...
        debug_log = DebugLog()
        check = DijkstraMethodGrid(find_path_data, debug_log)
        check.run()
        check.visualize()
