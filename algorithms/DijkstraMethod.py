...
class Node:
    def __init__(self, point, g, prev_node, coordinate_int_x, coordinate_int_y):
        self.point = point
        self.g = g
...
class DijkstraMethod(AlgorithmsBasedOnHallOnly, SearchAlgorithm, ABC):
    ...
    def __new_neighbor(self, node, x, y):
        ...
        if not [new_x, new_y] in self.all_nodes_list_coor:
            ...
            if self.hall.hall_polygon.distance(point_geometry) == 0 and (
                    self.multi_polygon_geometry.distance(
                        point_geometry) > self.point_search_distance_diagonal) or self.multi_polygon_geometry.isNull():
                if (x == 1 or x == -1) and (y == 1 or y == -1):
                    new_node = Node(point, node.g + self.point_search_distance_diagonal, node, new_x,
                                    new_y)
                else:
                    new_node = Node(point, node.g + self.point_search_distance, node, new_x, new_y)
    ...
    def __get_min_weight_node_in_open_list(self):
        return min(self.open_list, key=lambda p: p.g)
    ...
    def __start_searching(self):
        start_node = Node(self.starting_point, 0, None, 0, 0)
        ...
        while full_time < self.time_to_succeed:
            ...
            if self.__check_distance_to_target_point(current_node) < self.point_search_distance_diagonal:
                ...
                if self.multi_polygon_geometry.distance(line):
                    self.last_node = Node(self.target_point, 0, current_node, 0, 0)
                    self.is_succes = True
                    break
            self.open_list.remove(current_node)
            self.closed_list.append(current_node)
    ...
    def run(self):
        ...
        self.final_path = self._get_shorter_path(self.list_of_path, 2)
